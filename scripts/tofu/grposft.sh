        ...
        for lr in "${learning_rates[@]}"; do
            for task_id in "${task_list[@]}"; do
                for macro_epoch in "${macro_epochs[@]}"; do
                    COMMON="use_LoRA=$use_LoRA forget_coeff=$forget_coeff regularization_coeff=$regularization_coeff lr=$lr split=$split forget_loss=$forget_loss num_epochs=$num_epochs macro_epoch=$macro_epoch \
                    ...
                    CUDA_VISIBLE_DEVICES=1,0,3 torchrun --nproc_per_node=3 --master_port=$MASTER_PORT \
                        sftwithgrpo.py \
